...
#define ITEM_POS_X (4575)
#define ITEM_POS_Y (225)
...
GameObject* SetStoryItem9(GameObject* player, RACamera* camera)
{
	_player = player;
	_camera = camera;
	...
	SetImage(_storyItem->animArray, 0, 0, STORY_9_ITEM_0);

	SetAnimation(_storyItem->animArray, 1, 1000, 1);
	SetImage(_storyItem->animArray, 1, 0, STORY_9_ITEM_1);
	...
	SetImage(_storyWallPaper->animArray, 0, 0, STORY_WALL_PAPER_9);

	return _storyItem;
}

void DisableStoryItem9()
{
