...
GameObject* SetButtonMainMenuToExitGame()
{
	_buttonExitGame = AllocGameObject();

	_buttonExitGame->layer = LAYER_SHADOW;
	_buttonExitGame->posX = 960;
	_buttonExitGame->posY = 956;
	...
	SetImage(_buttonExitGame->animArray, 0, 0, M_BUTTON_EXIT_GAME);

	SetAnimation(_buttonExitGame->animArray, 1, 1000, 1);
	SetImage(_buttonExitGame->animArray, 1, 0, M_BUTTON_EXIT_GAME_SELECT);
