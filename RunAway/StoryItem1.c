...
#define ITEM_POS_X (2625)
#define ITEM_POS_Y (6975)
...
GameObject* SetStoryItem1(GameObject* player, RACamera* camera)
{
	_player = player;
	_camera = camera;
	...
	SetImage(_storyItem->animArray, 0, 0, STORY_1_ITEM_0);

	SetAnimation(_storyItem->animArray, 1, 1000, 1);
	SetImage(_storyItem->animArray, 1, 0, STORY_1_ITEM_1);
	...
	SetImage(_storyWallPaper->animArray, 0, 0, STORY_WALL_PAPER_1);

	return _storyItem;
}

void DisableStoryItem1()
{
